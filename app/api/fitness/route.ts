...
async function readData(supabase:Awaited<ReturnType<typeof createClient>>,uid:string,name:string):Promise<FitnessData>{
 ...
 const mockIds=rows.filter(row=>(row.resource==='session'&&(row.payload as {isDemo?:boolean})?.isDemo===true)||(row.resource==='routine'&&row.id.startsWith(`${uid}:routine-`))).map(row=>row.id);
 if(mockIds.length){const cleared=await supabase.from('fitness_resources').delete().eq('user_id',uid).in('id',mockIds);if(cleared.error)throw cleared.error;rows=rows.filter(row=>!mockIds.includes(row.id));}
 if(!rows.some(row=>row.resource==='profile')){const seed=initialData(uid,name);const resources=[{user_id:uid,id:uid,resource:'profile',payload:seed.profile,version:1},...seed.routines.map(payload=>({user_id:uid,id:payload.id,resource:'routine',payload,version:1})),...seed.sessions.map(payload=>({user_id:uid,id:payload.id,resource:'session',payload,version:1}))];const created=await supabase.from('fitness_resources').upsert(resources,{onConflict:'user_id,id'}).select('id,resource,payload,version');if(created.error)throw created.error;rows=(created.data??[]) as Row[];}
