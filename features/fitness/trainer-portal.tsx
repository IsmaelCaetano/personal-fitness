...
export function TrainerPortal() {
  ...
  async function action(body: Record<string, string>) {
    ...
    try {
      ...
      if (!response.ok) { setError(payload.error ?? 'Não foi possível concluir.'); return; }
      await load();
  ...
  return <div className="content-area trainer-portal" style={{ maxWidth: 1150, margin: 'auto', padding: 24 }}>
    ...
    {isTrainer && <>
      ...
      <section className="card"><h2>Pré-cadastrar aluno</h2><p>Informe nome e e-mail. Quem ainda não tem conta receberá um convite para definir sua senha; quem já tem conta aceita o convite ao entrar.</p><div className="trainer-invite-fields"><label>Nome do aluno<input value={studentName} maxLength={80} onChange={(event)=>setStudentName(event.target.value)} placeholder="Nome completo" /></label><label>E-mail do aluno<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="aluno@email.com" /></label></div><button className="primary" disabled={!email.includes('@')||!studentName.trim()} onClick={() => action({ action: 'invite', email, studentName: studentName.trim() })}>Enviar convite</button>{portal.invites.some(invite=>invite.status==='pending'&&invite.trainer_id===portal.viewerId)&&<div className="trainer-invite-list"><h3>Convites pendentes</h3>{portal.invites.filter(invite=>invite.status==='pending'&&invite.trainer_id===portal.viewerId).map(invite=><p key={invite.id}>{invite.student_name || 'Aluno'} · {invite.email} · expira em {new Date(invite.expires_at).toLocaleDateString('pt-BR')}</p>)}</div>}</section>
