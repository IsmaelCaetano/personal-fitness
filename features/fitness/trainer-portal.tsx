...
type Invite = { id: string; trainer_id: string; email: string; status: string; expires_at: string };
...
type Portal = { profile: { account_type: 'individual' | 'trainer'; display_name: string }; relationships: Link[]; invites: Invite[]; names: { studentId: string; name: string }[] };
...
export function TrainerPortal() {
  ...
  return <div className="content-area" style={{ maxWidth: 1150, margin: 'auto', padding: 24 }}>
    <Toaster />
    ...
    {isTrainer && <>
      ...
      <section className="card"><h2>Convidar aluno</h2><label>E-mail do aluno<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="aluno@email.com" /></label><button className="primary" disabled={!email.includes('@')} onClick={() => action({ action: 'invite', email })}>Enviar convite</button></section>
